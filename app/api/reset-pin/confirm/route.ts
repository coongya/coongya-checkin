...
// PIN 재설정 2단계: 인증 코드 확인 후 새 PIN 저장 + 로그인.
// 어떤 단계에서 틀렸는지 구분하지 않는 동일한 에러 메시지를 쓴다 (계정/코드 탐색 방지).
const GENERIC_ERROR = "인증 코드가 올바르지 않거나 만료됐어요.";
...
export async function POST(req: NextRequest) {
  ...
  if (!allowRequest(`resetcfm:${clientIp(req)}`, 10, 60_000)) {
  ...
  if (reset.attempts >= MAX_ATTEMPTS) {
    ...
    return NextResponse.json(
      { error: "시도 횟수를 초과했어요. 인증 코드를 다시 요청해 주세요." },
      { status: 429 }
    );
  }
