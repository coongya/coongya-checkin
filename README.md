...
- **그룹(로그)**: 그룹 생성 시 6자리 초대코드 발급, 초대코드로 참여
  — 대시보드·설정의 초대코드는 누르면 클립보드에 복사
- **간편 로그인**: 이메일 + PIN(숫자 4~6자리), HMAC 서명 쿠키 세션
  — 이메일이 계정의 유니크 키(로그인 ID), 닉네임은 표시용이라 중복 사용 가능
- **PIN 재설정**: PIN을 잊으면 이메일로 6자리 인증 코드를 받아 새 PIN 설정
  (코드는 해시 저장 · 10분 만료 · 5회 시도 제한 · 1회용)
...
- **PIN 재설정 보안**: 인증 코드는 scrypt 해시로만 저장, 10분 만료, 5회 시도 후
  폐기, 성공 시 즉시 1회용 폐기. 가입되지 않은 이메일에도 같은 응답을 돌려줘
  계정 존재 여부를 노출하지 않습니다
...
3. **Environment Variables**에 3개 입력:
   ...
   | `RESEND_API_KEY` | [resend.com](https://resend.com) API 키 — PIN 재설정 메일 발송용 |
   | `MAIL_FROM` | 발신자 주소 (선택, 기본 `onboarding@resend.dev`) |
...
lib/
  ...
  mailer.ts       # 이메일 발송 (Resend, 로컬은 콘솔 출력)
supabase/
