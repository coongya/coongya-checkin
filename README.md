...
- **그룹 관리**: 관리자가 그룹 이름·벌금·기록 시작일 변경, 그룹 삭제 가능
...
- **벌금**: 지각 1회 10,000원 · 무단 미출근 10,000원 (관리자가 금액 변경 가능 —
  바꾼 날부터 새 금액이 적용되고 지난 날짜는 그때 금액 그대로)
...
- **벌금은 저장하지 않고 파생 + 금액 변경 이력**: 일별 벌금은 저장하지 않고 기록으로
  계산하되, 금액은 `group_fine_history`의 "그 날짜에 유효했던 금액"을 씁니다 —
  금액을 바꿔도 과거 벌금이 다시 계산되지 않아요
